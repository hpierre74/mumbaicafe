...
class CreateBooking extends Component {
  ...
  sortService() {
    if (
      ...
      this.newBooking.time === "19:00" ||
      this.newBooking.time === "19:30"
    ) {
      this.newBooking.service = "dinner-1";
    } else if (
      this.newBooking.time === "21:30" ||
      this.newBooking.time === "22:00" ||
      this.newBooking.time === "22:30"
    ) {
  ...
  verifyServiceBookability(date, service, persons, isAdmin) {
    if (!this.props.isAdmin) {
      ...
      if (weekDay === 0) {
        this.setError(
          "danger",
          "Navrés, le Mumbai café n'ouvre pas le dimanche."
        );
        return false;
      }
      if (weekDay === 6 && service === "lunch") {
        this.setError(
          "danger",
          "Navrés, le restaurant est fermé le samedi midi."
        );
        return false;
      }
      if ((weekDay === 5 || weekDay === 6) && service !== "lunch") {
        this.setError(
          "danger",
          "Réservation indisponible les vendredis et samedi soirs."
        );
        return false;
      }
      ...
      if (date && service && persons) {
        ...
        if (this.state.bookabilities[date] === (undefined || null)) {
          ...
          Object.keys(this.state.bookabilities).map((booking, index) => {
            if (
              date === booking &&
              this.state.bookabilities[booking][service] !== (null || undefined)
            ) {
              Object.values(this.state.bookabilities[booking][service]).map(
                (value, index) => {
  ...
  restrictLateBookings(service, date) {
    ...
    if (service === "lunch") {
      if (
        moment(new Date()).isBefore(moment(new Date(booking_date + " 12:00")))
      ) {
        return true;
      } else {
        return false;
    ...
    if (service !== "lunch") {
      if (
        moment(new Date()).isBefore(moment(new Date(booking_date + " 18:30")))
      ) {
        return true;
      } else {
        return false;
  ...
  _handleBookingSubmit(event) {
    ...
    this.newBooking.bookStamp = !this.newBooking.bookStamp
      ? moment(new Date(this.newBooking.date + " " + this.newBooking.time)).unix()
      : this.newBooking.bookStamp;
    ...
    if (
      ...
      moment(new Date(this.newBooking.date)).dayOfYear() < moment().dayOfYear()
    ) {
      ...
      !this.restrictLateBookings(this.newBooking.service, this.newBooking.date)
    ) {
      this.setError(
        "danger",
        "Heure limite de réservation atteinte. Des tables sont peut être encore disponibles sur place."
      );
      return;
      ...
      this.newBooking.tel.length > 14 ||
      this.newBooking.tel.length < 10
    ) {
      this.setError(
        "danger",
        "Le numéro de téléphone doit faire entre 10 et 14 chiffres"
      );
      //alert('Le numéro de téléphone doit faire 10 chiffres minimum (06xxxxxxxx ou +33 6xxxxxxxx)');
      return;
    } else if (
      !this.props.isAdmin &&
      (this.newBooking.persons <= 0 || this.newBooking.persons > 15)
    ) {
      ...
      let newBookingKey = firebase
        .database()
        .ref()
        .child("booker/bookings")
        .push().key;
      let bookingData = {
        ...
        datetime: moment(new Date(this.newBooking.date + " " + this.newBooking.time)).format('DD-MM-YYYY HH:mm'),
      ...
      let updates = {};
      updates["/contacts/" + bookingData.name +'/email'] = bookingData.email;
      updates["/contacts/" + bookingData.name +'/tel'] = bookingData.tel;
      updates["/contacts/" + bookingData.name +'/bookings/'+ newBookingKey] = bookingData;
      ...
      this.verifyServiceBookability(
        ...
        ? firebase
            ...
            .catch(error => {
              ...
              this.setError(
                "danger",
                "Echec de la réservation, vérifiez que tous les champs de formulaires sont bien remplis."
              );
            })
        : this.setError(
            "warning",
            "La prise de réservation en ligne n'est pas disponible pour la date demandée, contactez le restaurant pour plus d'informations"
          );
    }
  }
          ...
          <Col>
            <div className="panel panel-booker">
              ...
              <Form
                ...
                onSubmit={e => this._handleBookingSubmit(e)}
              >
                <Row>
                  <Col xs="12" sm="6">
                    <FormGroup>
                ...
                <Row>
                  ...
                  <Col xs="12" sm="6">
                    <FormGroup>
                      ...
                      <FormText>
                        Plus nombreux ? Contactez nous au 09.86.35.07.43
                      </FormText>
                    </FormGroup>
                  </Col>
                </Row>
                  ...
                  <Col sm="12" xs="12">
                    ...
                    >
                      <small>
                        ...
                        <br /> Nous ne prenons pas de réservation le vendredi
                        soir et samedi soir.Fermé le samedi midi et dimanche.
                      </small>
                    </FormText>
                  </Col>
                </Row>
              ...
              <Collapse isOpen={this.state.collapse}>
                <Alert
                  style={{ width: " 95%", margin: "0 auto 2.5%" }}
                  isOpen={this.state.collapse}
                  toggle={e => this.toggle("collapse", !this.state.collapse)}
